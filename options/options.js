...
options.initGeneralSettings = function() {
	...
	$("#seckey").val(options.settings["seckey"]);
	$("#sechash").val(options.settings["sechash"]);
	...
	$("#portButton").click(function() {
		...
		if(isNaN(port) || portNumber < 10 || portNumber > 59999) {
	...
	$("#seckeyButton").click(function() {
		var seckey = $("#seckey").val();
		
		if($.trim(seckey) == "") {
			$("#seckey").closest(".control-group").addClass("error");
			alert("Security Key cannot be empty.\nNothing saved!");
			return;
		}

		options.settings["seckey"] = seckey;
		$("#seckey").closest(".control-group").removeClass("error").addClass("success");
		setTimeout(function() {$("#seckey").closest(".control-group").removeClass("success")}, 2500);
	...
	$("#sechashButton").click(function() {
		var sechash = $("#sechash").val();
		
		if($.trim(sechash) == "") {
			$("#sechash").closest(".control-group").addClass("error");
			alert("Security Hash cannot be empty.\nNothing saved!");
			return;
		}

		options.settings["sechash"] = sechash;
		$("#sechash").closest(".control-group").removeClass("error").addClass("success");
		setTimeout(function() {$("#sechash").closest(".control-group").removeClass("success")}, 2500);
